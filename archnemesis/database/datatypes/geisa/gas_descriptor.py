...
from ...mappings.geisa import radtran_to_geisa, geisa_to_radtran
...
class GEISAGasDescriptor(NamedTuple):
    gas_id : int
    iso_id : int
    ...
    def from_radtran(cls, rt_gas_desc : RadtranGasDescriptor):
        result = radtran_to_geisa.get((rt_gas_desc.gas_id,rt_gas_desc.iso_id), None) # NOTE: Assume EXOMOL uses same gas numbering as GEISA
        if result is None:
            _lgr.warning(f'Could not convert {rt_gas_desc} to GEISA.')
        return result if result is None else cls.from_gas_and_iso_id(*result)
    
    def to_radtran(self):
        result = geisa_to_radtran.get((self.gas_id, self.iso_id), None)
        if result is None:
